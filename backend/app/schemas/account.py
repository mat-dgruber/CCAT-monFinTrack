...
class AccountType(str, Enum):
    ...
    CREDIT_CARD = (
        "credit_card"  # Cartão de Crédito (sim, cartão pode ser uma 'conta' negativa)
    )
    OTHER = "other"
...
class CreditCard(BaseModel):
    ...
    brand: CreditCardBrand = Field(
        default=CreditCardBrand.OTHER, description="Bandeira"
    )
    limit: float = Field(default=0.0, description="Limite do cartão")
    closing_day: int = Field(
        ..., ge=1, le=31, description="Dia de fechamento da fatura"
    )
    invoice_due_day: int = Field(
        ..., ge=1, le=31, description="Dia de vencimento da fatura"
    )
...
class AccountBase(BaseModel):
    name: str = Field(
        ..., min_length=2, description="Nome da Conta (ex: Nubank, C6 Bank, etc"
    )
    ...
    credit_cards: List[CreditCard] = Field(
        default=[], description="Cartões de crédito vinculados"
    )
