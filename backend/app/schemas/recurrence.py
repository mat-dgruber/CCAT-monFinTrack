...
class RecurrenceBase(BaseModel):
    name: str = Field(
        ..., min_length=3, description="Nome da recorrência (ex: Netflix)"
    )
    ...
    payment_method_id: Optional[str] = Field(
        None, description="ID da forma de pagamento"
    )
    credit_card_id: Optional[str] = Field(
        None, description="ID do cartão de crédito (se aplicável)"
    )
    periodicity: RecurrencePeriodicity = Field(..., description="Periodicidade")
    auto_pay: bool = Field(
        False, description="Se true, gera como PAGO. Se false, PENDENTE."
    )
    due_day: int = Field(..., ge=1, le=31, description="Dia base de vencimento")
    due_month: Optional[int] = Field(
        None, ge=1, le=12, description="Mês de vencimento para recorrências anuais"
    )
    active: bool = Field(True, description="Se a recorrência está ativa")
    skipped_dates: List[date] = Field(
        default_factory=list,
        description="Lista de datas (vencimentos) puladas/excluídas",
    )
    start_date: Optional[date] = Field(
        None, description="Data de início da recorrência (para simulações retroativas)"
    )

    # Novo campo para suportar Transferência (Pagamento de Fatura)
    type: TransactionType = Field(
        default=TransactionType.EXPENSE, description="Tipo da transação gerada"
    )
