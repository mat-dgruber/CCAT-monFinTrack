...
class TransactionBase(BaseModel):
    ...
    date: datetime = Field(
        default_factory=datetime.now, description="Data da transação"
    )
    type: TransactionType = Field(
        default=TransactionType.EXPENSE, description="Tipo: Despesa ou Receita"
    )
    payment_method: PaymentMethod = Field(..., description="Forma de pagamento")
    status: TransactionStatus = Field(
        default=TransactionStatus.PAID, description="Status da transação"
    )
    payment_date: Optional[datetime] = Field(
        None, description="Data efetiva do pagamento"
    )
    ...
    is_auto_pay: bool = Field(
        False, description="Se o pagamento é automático (via recorrência)"
    )

    # Se a transação for feita via cartão de crédito específico
    credit_card_id: Optional[str] = Field(
        None, description="ID do cartão de crédito aninhado"
    )

    # Destination Account for Transfers
    destination_account_id: Optional[str] = Field(
        None, description="ID da conta de destino (apenas para transferências)"
    )
    ...
    offering_percentage: Optional[float] = Field(
        None, description="Porcentagem da oferta"
    )
    net_amount: Optional[float] = Field(None, description="Valor líquido (lucro)")
    tithe_status: Optional[str] = Field(
        None, description="Status do dízimo: NONE, PENDING, PAID"
    )
    gross_amount: Optional[float] = Field(
        None, description="Valor bruto da transação (antes das deduções)"
    )
    ...
    warning: Optional[str] = Field(
        None, description="Avisos de anomalias (gastos excessivos)"
    )

    # Attachments (Firebase Storage URLs)
    attachments: Optional[List[str]] = Field(
        default=[], description="URLs de comprovantes ou anexos"
    )
...
class TransactionCreate(TransactionBase):
    ...
    installment_group_id: Optional[str] = Field(
        None, description="ID do grupo de parcelamento"
    )
    installment_number: Optional[int] = Field(
        None, description="Número da parcela (ex: 1)"
    )

    total_installments: Optional[int] = Field(
        None, description="Total de parcelas (ex: 10)"
    )

    # Campos para criar Recorrência (Cenário B)
    is_recurrence: bool = Field(
        False, description="Se true, cria uma recorrência ao invés de transação única"
    )
    recurrence_periodicity: Optional[str] = Field(
        None, description="Periodicidade (mensal, anual...)"
    )
    recurrence_auto_pay: bool = Field(False, description="Pagamento automático?")
    recurrence_create_first: bool = Field(
        True, description="Gerar a primeira transação agora?"
    )
...
class TransactionUpdate(BaseModel):
    ...
    type: Optional[TransactionType] = Field(
        None, description="Tipo: Despesa ou Receita"
    )
    payment_method: Optional[PaymentMethod] = Field(
        None, description="Forma de pagamento"
    )
