...
def calculate_monthly_averages_helper(
    transactions: List[Any], months_count: float
) -> Dict[str, Any]:
    """
    ...
    return {"total_avg": avg_total, "by_category": avg_by_category}
...
def get_projection_inflation() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"rate": round(final_rate, 2), "is_fallback": False, "message": ""}

    except Exception as e:
        logger.warning("Error fetching inflation API: %s", e)
        return default_response
...
def get_monthly_averages(
    ...
    if tier == "free":
        raise HTTPException(
            status_code=403,
            detail="Cost of Living Analysis is available for Pro and Premium users.",
        )
    ...
    transactions = transaction_service.list_transactions(
        user_id=user_id, start_date=dt_start, end_date=dt_end, limit=10000
    )
    ...
    variable_data = calculate_monthly_averages_helper(
        variable_transactions, months_count
    )
    ...
    return {
        ...
        "realized": {
            "average_total": variable_data[
                "total_avg"
            ],  # This is now just the VARIABLE part
            "by_category": variable_data["by_category"],
        },
        "committed": {"total": committed_total},
...
def check_subscriptions(current_user: dict = Depends(get_current_user)):
    ...
    if tier != "premium":
        raise HTTPException(
            status_code=403,
            detail="Anomaly/Subscription Detection is available only for Premium users.",
        )
...
def check_anomalies(
    ...
    if tier == "free":
        raise HTTPException(
            status_code=403,
            detail="Anomaly Detection is available for Pro and Premium users.",
        )
    ...
    if target_month == 12:
        ...
        dt_end_target = datetime(target_year, target_month + 1, 1) - timedelta(
            microseconds=1
        )
            ...
            anomalies.append(
                {
                    ...
                    "average": avg_val,
                    "pct_increase": (
                        ((current_val - avg_val) / avg_val) * 100
                        if avg_val > 0
                        else 100
                    ),
                    "severity": severity,
