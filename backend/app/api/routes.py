...
@limiter.limit("10 per minute")
def delete_account(
    request: Request, account_id: str, current_user: dict = Depends(get_current_user)
):
...
def read_categories(
    type: Optional[CategoryType] = None, current_user: dict = Depends(get_current_user)
):
...
@limiter.limit("10 per minute")
def delete_category(
    request: Request, category_id: str, current_user: dict = Depends(get_current_user)
):
...
def read_upcoming_transactions(
...
# --- DÍZIMOS ---
@router.get("/transactions/tithes/pending")
def read_pending_tithes(current_user: dict = Depends(get_current_user)):
    """Lista todas as receitas com dízimo pendente."""
    transactions = transaction_service.list_pending_tithes(current_user["uid"])
    return [t.model_dump() for t in transactions]


@router.patch("/transactions/tithes/pay-all")
@limiter.limit("5 per minute")
def pay_all_tithes(request: Request, current_user: dict = Depends(get_current_user)):
    """Marca todos os dízimos pendentes como pagos."""
    return transaction_service.batch_pay_tithes(current_user["uid"])
...
@limiter.limit("10 per minute")
def delete_budget(
    request: Request, budget_id: str, current_user: dict = Depends(get_current_user)
):
...
def list_recurrences(
    ...
    return recurrence_service.list_recurrences(
        current_user["uid"], active_only=active_only
    )
...
def update_recurrence(
    ...
    return recurrence_service.update_recurrence(
        recurrence_id, recurrence, current_user["uid"], scope
    )


@router.patch("/recurrences/{recurrence_id}/cancel", response_model=Recurrence)
def cancel_recurrence(
    recurrence_id: str, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        if isinstance(skip_date_str, str):
            ...
            if "T" in skip_date_str:
                skip_date = datetime.fromisoformat(
                    skip_date_str.replace("Z", "+00:00")
                ).date()
            else:
        ...
        recurrence = recurrence_service.get_recurrence(
            recurrence_id, current_user["uid"]
        )
        if not recurrence:
...
router.include_router(
    user_preference.router, prefix="/preferences", tags=["User Preferences"]
)
