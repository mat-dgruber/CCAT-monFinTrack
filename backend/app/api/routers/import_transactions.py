...
from app.utils.parsers import parse_csv, parse_ofx
...
async def preview_import(
    ...
    if tier == "free":
        raise HTTPException(
            status_code=403,
            detail="Transaction Import is available for Pro and Premium users.",
        )
    ...
    if filename.endswith(".ofx"):
        ...
        raise HTTPException(
            status_code=400,
            detail="Unsupported file format. Please upload .ofx or .csv",
        )

    if not transactions:
        raise HTTPException(
            status_code=400, detail="Could not parse transactions from file."
        )
    ...
    for tx in transactions:
        ...
        if tx["type"] == "expense":  # Usually only categorize expenses
            cat_id = ai_service.classify_transaction(
                tx["description"], current_user["uid"]
            )
        ...
        drafts.append(
            DraftTransaction(
                ...
                amount=abs(
                    tx["amount"]
                ),  # Front prefers absolute for table? Let's keep raw or abs? Usually separate columns.
