...
@router.post("/enable")
def enable_mfa(data: MFAVerifyRequest, current_user: dict = Depends(get_current_user)):
    """
    Verifica o token e ativa o MFA para o usuário.
    """
    if not data.secret:
        raise HTTPException(
            status_code=400, detail="Secret is required for enabling MFA"
        )
...
def verify_mfa_login(
    data: MFALoginVerifyRequest, current_user: dict = Depends(get_current_user)
):
    """
