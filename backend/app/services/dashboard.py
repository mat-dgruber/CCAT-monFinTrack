...
def get_dashboard_data(
    ...
    # Normalize Timezones
    if evolution_start.tzinfo is None:
        evolution_start = evolution_start.replace(tzinfo=timezone.utc)
    ...
    if accounts:
        current_transactions = [
            t for t in current_transactions if t.get("account_id") in accounts
        ]
    ...
    for cat_id, total in category_map.items():
        ...
        if cat_obj:
            categories_list.append(
                CategoryTotal(
                    category_name=cat_obj.name, color=cat_obj.color, total=total
                )
            )
        else:
            ...
            categories_list.append(
                CategoryTotal(
                    category_name=(
                        "Sem Categoria" if cat_id == "unknown" else "Categoria Excluída"
                    ),
    ...
    for m, y in months_to_fetch:
        ...
        evolution_data.append(
            MonthlyEvolution(month=label, income=m_income, expense=m_expense)
        )
