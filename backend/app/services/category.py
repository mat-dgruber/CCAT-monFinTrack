...
def ensure_default_categories(user_id: str):
    ...
    defaults = [
        ...
        },
        {
            ...
            "subcategories": [
                ...
                {
                    "name": "Bares/Festas",
                    "icon": "pi pi-glass-cheers",
                    "color": "#60A5FA",
                    "type": CategoryType.EXPENSE,
                },  # glass-cheers not in primeicons v4, using alternative?
            ],
        },
        {
            "name": "Compras",
        ...
        },
        {
            ...
            "color": "#A78BFA",
            "type": CategoryType.EXPENSE,
            "subcategories": [  # pi-github as placeholder for paw/pet if not available
                {
                    "name": "Ração",
                    "icon": "pi pi-box",
                    "color": "#A78BFA",
    ...
    for cat_data in defaults:
        if cat_data["name"] == "Fatura Cartão":
            continue  # Já tratada acima (ou poderia ser tratada aqui, mas ok manter separado)
...
def list_categories(
    user_id: str, cat_type: Optional[CategoryType] = None
) -> List[Category]:
    db = get_db()
...
def list_all_categories_flat(user_id: str) -> List[Category]:
    """
    Retorna TODAS as categorias do usuário em lista plana (sem hierarquia).
...
def delete_category(category_id: str, user_id: str):
    ...
    if any(children_query):
        raise HTTPException(
            status_code=400,
            detail="Cannot delete category with subcategories. Please delete or move them first.",
        )
...
def get_category(category_id: str, user_id: str = None):
    if not category_id:
        return None
    db = get_db()
    ...
    if doc.exists:
        ...
        if user_id and data.get("user_id") != user_id:
            return None
        return Category(id=doc.id, **data)
    return None
