...
from app.schemas.invoice import InvoiceStatus, InvoiceSummary
    ...
    for acc in accounts:
        if acc.credit_cards:
            for card in acc.credit_cards:
                cards_map[card.id] = {"card": card, "account": acc}
...
def pay_invoice(user_id: str, invoice_data: dict):
    ...
    description_text = invoice_data.get("description", "Pagamento Fatura")
    full_description = f"{description_text} | {ref_key}"

    if not source_account_id:
        raise HTTPException(
            status_code=400, detail="Conta de origem (source_account_id) obrigatória"
        )
    ...
    if not category_id:
        ...
        created_cat = transaction_service.category_service.create_category(
            new_cat, user_id
        )
    ...
    if not category_id:
        raise HTTPException(
            status_code=400, detail="Não foi possível definir a categoria da fatura."
        )
