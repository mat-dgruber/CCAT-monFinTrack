...
def list_recurrences(user_id: str, active_only: bool = False) -> List[Recurrence]:
    ...
    for doc in docs:
        try:
            ...
            logger.warning("Error parsing recurrence %s: %s", doc.id, e)
...
def update_recurrence(
    recurrence_id: str,
    recurrence_in: RecurrenceUpdate,
    user_id: str,
    scope: str = "all",
) -> Recurrence:
    db = get_db()
    ...
    if scope == "future":
        ...
        if "start_date" in update_data and update_data["start_date"] is not None:
            update_data["start_date"] = (
                ...
                if hasattr(update_data["start_date"], "isoformat")
                else update_data["start_date"]
            )
