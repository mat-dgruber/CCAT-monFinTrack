...
def test_create_checkout_existing_customer(mock_session_create, stripe_service, mock_db):
    ...
    result = stripe_service.create_checkout_session("user_123", "pro_monthly", "http://success", "http://cancel")
...
@patch("app.services.stripe_service.stripe.checkout.Session.create")
def test_create_checkout_new_customer(mock_session_create, mock_customer_create, stripe_service, mock_db):
    ...
    result = stripe_service.create_checkout_session("user_123", "premium_monthly", "http://success", "http://cancel")
    
    assert result == {"sessionId": "cs_123", "url": "http://checkout.com"}
    mock_customer_create.assert_called_once_with(metadata={"user_id": "user_123"})
    # verify save customer id to db
    mock_db.collection().document().set.assert_called_with({"stripe_customer_id": "cus_new_123"}, merge=True)
...
def test_create_portal_no_customer(mock_portal_create, stripe_service, mock_db):
    ...
    assert exc_info.value.status_code == 400
    assert "stripe customer id" in exc_info.value.detail.lower()
...
def test_handle_checkout_completed(mock_retrieve, mock_handle_updated, stripe_service, mock_db):
    session = {
