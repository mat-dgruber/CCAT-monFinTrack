...
def test_filters():
    ...
    if all_txs:
        ...
        filtered_txs = transaction_service.list_transactions(
    ...
    print(
        f"Dashboard Income (All Time/Default): {dash_all.income_month}"
    )  # Note: Logic changed to filter, so without filter it might be all time or just fail if logic expects filter for "month" income?
    ...
    if all_txs:
        ...
        dash_filtered = dashboard_service.get_dashboard_data(
            user_id, month=month, year=year
        )
    ...
    for b in budgets:
        print(
            f"  - {b['category'].name}: Limit {b['amount']}, Spent {b['spent']}, % {b['percentage']:.1f}"
        )
