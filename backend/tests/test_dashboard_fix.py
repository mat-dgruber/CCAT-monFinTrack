...
from app.schemas.transaction import PaymentMethod, Transaction, TransactionType
...
def test_get_dashboard_data_handles_string_dates(
    mock_budget_service, mock_category_service, mock_get_db
):
    """
    ...
    def collection_side_effect(collection_name):
        ...
        if collection_name == "accounts":
            ...
            chain = (
                coll_mock.where.return_value.where.return_value.order_by.return_value.limit.return_value
            )
            ...
            coll_mock.where.return_value.where.return_value.stream.return_value = [
                mock_transaction
            ]
            coll_mock.where.return_value.where.return_value.order_by.return_value.stream.return_value = [
                mock_transaction
            ]
        elif collection_name == "categories":
            coll_mock.where.return_value.where.return_value.limit.return_value.stream.return_value = [
                mock_invoice_cat
            ]
            coll_mock.where.return_value.limit.return_value.stream.return_value = [
                mock_invoice_cat
            ]
        return coll_mock
    ...
    with patch(
        "app.services.dashboard.transaction_service.list_transactions"
    ) as mock_list:
