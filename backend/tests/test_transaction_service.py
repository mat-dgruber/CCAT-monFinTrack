...
def mock_external_services():
    with patch("app.services.transaction.category_service") as cat_mock, patch(
        ...
        "app.services.transaction.recurrence_service"
    ) as rec_mock, patch(
        "app.services.transaction._update_account_balance"
    ) as balance_mock:
...
def test_create_transaction_simple(mock_db, mock_external_services):
    ...
    # Mocks
    cat_mock.get_category.return_value = Category(
        id="cat1",
        name="Food",
        type="expense",
        icon="",
    ...
    acc_mock.get_account.return_value = Account(
        id="acc1", name="Bank", type="checking", balance=100, user_id=user_id
    )
...
def test_create_unified_transaction_installments(mock_db, mock_external_services):
    ...
    acc_mock.get_account.return_value = Account(
        id="acc1", name="Bank", type="checking", balance=1000, user_id=user_id
    )
