...
export class AIService {
  ...
  classifyTransaction(
    description: string,
  ): Observable<{ category_id: string | null }> {
    return this.http.post<{ category_id: string | null }>(
  ...
  generateMonthlyReport(
    ...
    return this.http.get<{ content: string }>(`${this.apiUrl}/report`, {
      params: { month, year },
    });
  }
  ...
  analyzeCostOfLiving(data: any): Observable<{ analysis: string }> {
    ...
    return this.http.post<{ analysis: string }>(
      `${this.apiUrl}/cost-of-living-analysis`,
      { data },
    );
  }
