...
export class AccountManager implements OnInit {
  ...
  canAccessCreditCardMgmt = computed(() =>
    this.subscriptionService.canAccess('credit_card_mgmt'),
  );
  ...
  saveAccount() {
    if (this.form.valid) {
      ...
      if (this.editingId()) {
        this.accountService
          .updateAccount(this.editingId()!, payload)
          .subscribe(onSave);
      } else {
    ...
    if (this.transferForm.valid && this.sourceAccount()) {
      ...
      this.categoryService.getCategories().subscribe((categories) => {
        ...
        let transferCategory = categories.find(
          (c) => c.name === 'Transferencia entre contas',
        );
        if (!transferCategory) {
        ...
        if (!transferCategory) {
          this.messageService.add({
            severity: 'error',
            summary: 'Erro',
            detail: 'Categoria de transferência não encontrada.',
          });
          return;
        }
